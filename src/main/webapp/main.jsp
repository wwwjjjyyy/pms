...
<link rel="stylesheet" type="text/css" href="skin/css/main.css" />
	...
	<script type="text/javascript">
		$(function () {
			$.ajax({
				type:"get",
				url:"${pageContext.request.contextPath}/notice/recently",
				success:function (msg) {
					$(msg.map.list).each(function (index,item) {
						var li = "<li class='ue-clear'>"
                            	+"<span>"+moment(item.ndate).format("YYYY-MM-DD")+"</span>&nbsp;&nbsp;&nbsp;<a onclick='showWindow("+item.nid+")' href='javascript:void(0)'class='notice-title'>"+item.ntitle+"</a>"
                        		+"</li><p>";
						$("#ul").append(li);
                    })
                }
		...
		function showWindow(nid) {
            $.ajax({
                type:"GET",
                url:"${pageContext.request.contextPath}/notice/getById",
                data:{"nid":nid},
                success:function (msg) {
                    if (msg.map.statusCode == 200){
                        $("#ntitle").text(msg.map.notice.ntitle);
                        $("#content").text(msg.map.notice.remark).css({"font-size":"30px"});
                        $("#showdiv").show();
...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<!-- 按钮 -->
	<div onclick="closeWindow()" style="background: green; width: 10%; margin: 0 auto; height: 1.5rem; line-height: 1.5rem; text-align: center;color: #fff;margin-top: 28rem; -moz-border-radius: .128rem; -webkit-border-radius: .128rem; border-radius: .128rem;font-size: .59733rem;" onclick="closeWindow()">
		关闭
	</div>
</div>
